...
def test_ffmpeg_zero_frame_diagnostics_include_decoder_and_counts(caplog):
    from model_service.video.frame_extractor import (
...
def test_ffprobe_invalid_data_uses_short_polling_without_long_sleep(monkeypatch, tmp_path):
    import subprocess

    import model_service.video.frame_extractor as frame_extractor_module
    from model_service.video.frame_extractor import StreamingFrameExtractor

    video_path = tmp_path / "still-writing.avi"
    video_path.write_bytes(b"partial avi")
    ...
    monkeypatch.setattr(frame_extractor_module.subprocess, "run", fake_run)
    monkeypatch.setattr(frame_extractor_module.time, "sleep", fake_sleep)
    monkeypatch.setattr(frame_extractor_module.time, "monotonic", lambda: elapsed[0])

    extractor = StreamingFrameExtractor(str(video_path), use_hwaccel=False)

    assert extractor._probe_video() is False
    assert run_calls
    assert sleep_calls
...
def test_loadcell_analysis_uses_plateaus_not_tail_transient_extremes():
    from model_service.core import loadcell_stats

    series = _make_weight_series(
        [1000.0] * 5 + [900.0, 800.0, 700.0] + [480.0] * 5 + [430.0]
    )
    ...
    assert analysis.delta == pytest.approx(-520.0)
    assert analysis.decision_delta == pytest.approx(-520.0)
    assert analysis.reason == "stable_regions"
    assert analysis.stable_delta_source == "confirmed_stable_plateaus"
    assert analysis.baseline_stable_avg == pytest.approx(1000.0)
    assert analysis.final_stable_avg == pytest.approx(480.0)
    assert analysis.trailing_unstable_sample_count == 1
    assert analysis.raw_simple_delta == pytest.approx(-570.0)
    assert analysis.raw_extreme_delta == pytest.approx(-570.0)


def test_loadcell_analysis_rejects_long_unstable_tail_after_final_plateau():
    from model_service.core import loadcell_stats
    ...
    assert analysis.stable_delta_source == "stable_tail_not_confirmed"
    assert analysis.baseline_stable_avg == pytest.approx(1000.0)
    assert analysis.final_stable_avg == pytest.approx(480.0)
    assert analysis.trailing_unstable_sample_count == 5
    assert analysis.raw_simple_delta == pytest.approx(-570.0)
    assert analysis.raw_extreme_delta == pytest.approx(-570.0)


def test_loadcell_analysis_marks_short_sample_simple_fallback():
    from model_service.core import loadcell_stats

    short_series = _make_weight_series([1000.0, 1007.6])
    ...
    assert analysis.delta == pytest.approx(7.6)
    assert analysis.start_avg == pytest.approx(1000.0)
    assert analysis.end_avg == pytest.approx(1007.6)
    assert analysis.parsed_sample_count == 2
    assert analysis.working_sample_count == 0
    assert analysis.used_simple_fallback is True
