...
class TestCrossZoneReturn:
    ...
    def test_cross_zone_return_after_source_zone_removal_preserves_source_purchase(
        ...
        """다른 zone 상품을 반환해도 같은 source zone의 별도 제거 상품은 유지."""
        store.add_trigger(
            zone=1,
        ...
        store.add_trigger(zone=2, result=self._create_return_trigger(zone=2, delta=365.0))

        zone1_session = store.get_session(zone=1)
        zone2_session = store.get_session(zone=2)

        assert zone1_session.aggregated_products[26].count == 0
        assert zone2_session.aggregated_products[27].count == 1
        assert zone2_session.unmatched_returns == []
        assert len(zone2_session.cross_zone_returns) == 1
        assert zone2_session.cross_zone_returns[0].target_zone == 1
        assert zone2_session.cross_zone_returns[0].product_name == "치킨마요"

    def test_cross_zone_return_survives_target_zone_reaggregation(self, store):
        """target zone에 이후 trigger가 추가되어도 기존 크로스존 반납을 재적용."""
        store.add_trigger(
            zone=1,
            result=self._create_removal_trigger(
                zone=1, delta=-365.0, product_id=26, name="치킨마요", count=1
    ...
    def test_cross_zone_return_matches_multiple_units_in_one_trigger(self, store):
        """동일 상품 여러 개를 한 번에 다른 zone에 반납해도 모두 차감."""
        store.add_trigger(
            zone=1,
            result=self._create_removal_trigger(
                zone=1, delta=-730.0, product_id=26, name="치킨마요", count=2
        ...
        store.add_trigger(zone=2, result=self._create_return_trigger(zone=2, delta=730.0))
        ...
        assert zone1_session.aggregated_products[26].count == 0
        assert zone2_session.unmatched_returns == []
        assert len(zone2_session.cross_zone_returns) == 2
        assert sum(r.delta_weight for r in zone2_session.cross_zone_returns) == 730.0

    def test_cross_zone_return_matches_multi_product_combo_from_one_zone(self, store):
        """여러 상품 조합을 한 번에 다른 zone에 반납하면 모두 복구."""
        store.add_trigger(
            zone=1,
            result=self._create_removal_trigger(
                zone=1, delta=-365.0, product_id=26, name="치킨마요", count=1
        ...
        store.add_trigger(zone=2, result=self._create_return_trigger(zone=2, delta=615.0))

        zone1_session = store.get_session(zone=1)
        zone2_session = store.get_session(zone=2)

        assert zone1_session.aggregated_products[26].count == 0
        assert zone1_session.aggregated_products[27].count == 0
        assert zone2_session.unmatched_returns == []
        assert len(zone2_session.cross_zone_returns) == 2
        assert {r.product_name for r in zone2_session.cross_zone_returns} == {
            "치킨마요",
            "참치마요",
        }

    def test_cross_zone_return_matches_combo_from_multiple_zones(self, store):
        """여러 zone에서 꺼낸 상품을 한 zone에 같이 반납하면 각 zone을 복구."""
        store.add_trigger(
            zone=1,
            result=self._create_removal_trigger(
                zone=1, delta=-365.0, product_id=26, name="치킨마요", count=1
        ...
        store.add_trigger(zone=3, result=self._create_return_trigger(zone=3, delta=615.0))
        ...
        assert zone3_session.unmatched_returns == []
        assert len(zone3_session.cross_zone_returns) == 2
        assert {r.target_zone for r in zone3_session.cross_zone_returns} == {1, 2}

    def test_three_cross_zone_recoveries_do_not_block_next_detection(self, store):
        """3회 이상 복구 후 다음 제거 trigger도 정상 반영."""
        store.add_trigger(
            zone=1,
            result=self._create_removal_trigger(
                zone=1, delta=-1095.0, product_id=26, name="치킨마요", count=3
            ),
        )
        store.add_trigger(zone=2, result=self._create_return_trigger(zone=2, delta=365.0))
        store.add_trigger(zone=3, result=self._create_return_trigger(zone=3, delta=365.0))
        store.add_trigger(zone=4, result=self._create_return_trigger(zone=4, delta=365.0))
        ...
        assert zone1_session.aggregated_products[27].count == 1
        assert sum(
            len(store.get_session(zone).cross_zone_returns)
            for zone in (2, 3, 4)
        ) == 3
