...
class TestLoadcellPayloadDiagnostics:
    """Loadcell payload diagnostics for field 0g investigation."""

    def test_empty_payload_is_marked_empty(self):
        from model_service.core.loadcell_stats import summarize_loadcell_payload

        diagnostics = summarize_loadcell_payload([])

        assert diagnostics["sample_count"] == 0
        assert diagnostics["payload_state"] == "empty_payload"
        assert diagnostics["filtered_channel_count"] == 0
        assert diagnostics["filtered_parsed_channel_count"] == 0
        assert diagnostics["first_filtered_total"] is None
        assert diagnostics["last_filtered_total"] is None

    def test_all_zero_filtered_channels_are_marked_all_zero(self):
        from model_service.core.loadcell_stats import summarize_loadcell_payload
        ...
        diagnostics = summarize_loadcell_payload(loadcells)

        assert diagnostics["payload_state"] == "all_zero"
        assert diagnostics["filtered_channel_count"] == 4
        assert diagnostics["filtered_parsed_channel_count"] == 4
        assert diagnostics["filtered_zero_channel_count"] == 4
        assert diagnostics["filtered_nonzero_channel_count"] == 0
        assert diagnostics["first_filtered_total"] == 0.0
        assert diagnostics["last_filtered_total"] == 0.0

    def test_invalid_filtered_channels_are_marked_invalid_only(self):
        from model_service.core.loadcell_stats import summarize_loadcell_payload

        loadcells = [
            LoadcellData(
                timestamp="2026-05-28T10:00:00.000Z",
                raw_value=["EEEEEE", "VVVVVV"],
                filtered_value=["EEEEEE", "bad"],
                filter_method="exponential",
            )
        ]
        ...
        assert diagnostics["payload_state"] == "invalid_only"
        assert diagnostics["filtered_channel_count"] == 2
        assert diagnostics["filtered_parsed_channel_count"] == 0
        assert diagnostics["filtered_invalid_channel_count"] == 2
        assert diagnostics["first_filtered_values"] == ["EEEEEE", "bad"]
        assert diagnostics["last_filtered_values"] == ["EEEEEE", "bad"]

    def test_nonzero_payload_keeps_first_and_last_totals(self):
        from model_service.core.loadcell_stats import summarize_loadcell_payload

        loadcells = [
            LoadcellData(
                timestamp="2026-05-28T10:00:00.000Z",
                raw_value=["+01000", "+02000"],
                filtered_value=["+01000", "+02000"],
                filter_method="exponential",
            ),
            LoadcellData(
                timestamp="2026-05-28T10:00:01.000Z",
                raw_value=["+00900", "+01800"],
                filtered_value=["+00900", "+01800"],
                filter_method="exponential",
            ),
        ]
        ...
        assert diagnostics["payload_state"] == "nonzero"
        assert diagnostics["filtered_channel_count"] == 4
        assert diagnostics["filtered_nonzero_channel_count"] == 4
        assert diagnostics["first_raw_values"] == ["+01000", "+02000"]
        assert diagnostics["last_raw_values"] == ["+00900", "+01800"]
        assert diagnostics["first_filtered_total"] == 3000.0
        assert diagnostics["last_filtered_total"] == 2700.0
