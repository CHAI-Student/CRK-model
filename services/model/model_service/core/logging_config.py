"""
Structured logging configuration for Model Service module.
...
OPS_LOGGER_NAME = "model_service.ops"
...
class ConsoleOpsFilter(logging.Filter):
    """Allow concise operator logs plus warnings/errors on the console."""

    def filter(self, record: logging.LogRecord) -> bool:
        if record.levelno >= logging.WARNING:
            return True
        return record.name.startswith(OPS_LOGGER_NAME)
...
def setup_logging(log_level: str = "INFO") -> None:
    ...
    # The console receives concise operator lines plus warnings/errors so field
    # runs stay readable. Per-trigger detailed diagnostics are written by
    # TriggerTraceContext, not by an always-on root file handler.
    ...
    ops_logger = logging.getLogger(OPS_LOGGER_NAME)
...
def get_logger(name: str) -> logging.Logger:
    ...
    """
    return logging.getLogger(f"model.{name}")


def get_ops_logger() -> logging.Logger:
    """Get the concise operator-facing logger."""
