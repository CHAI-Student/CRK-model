...
class CandidateProduct:
    ...
    def has_motion_evidence(self) -> bool:
        return bool(
            self.top_motion_passed
            or self.side_motion_passed
            or self.motion_gate_passed
        )

    @property
    def evidence_score(self) -> float:
        score = min(max(float(self.vision_confidence), 0.0), 1.0)
        if self.has_motion_evidence:
            score += 0.20
        if self.vote_count >= 2:
            score += 0.08
        score += min(0.08, max(0, int(self.raw_vote_count)) * 0.01)
        if self.source == "stage_counts":
            score += 0.12
        elif self.source in {"threshold_rescue", "roi_rescue"}:
            score += 0.06 if self.weight_gate_passed else -0.08
...
class ValidCombination:
    ...
    def kind_count(self) -> int:
        """Number of distinct product kinds in the combination."""
        return len(self.items)

    @property
    def adjusted_score(self) -> float:
        """Score after penalizing broader, riskier combinations."""
        return (
            self.match_score
            - 0.35 * (self.total_count - 1)
            - 0.25 * (self.kind_count - 1)
        )
    ...
    def evidence_score(self) -> float:
        if self.total_count <= 0:
            return 0.0
        return sum(
            item.candidate.evidence_score * item.count
            for item in self.items
        ) / self.total_count

    @property
    def motion_evidence_ratio(self) -> float:
        if self.total_count <= 0:
            return 0.0
        motion_units = sum(
            item.count
            for item in self.items
            if item.candidate.has_motion_evidence
        )
        return motion_units / self.total_count
    ...
    def to_dict(self) -> dict:
        ...
        return {
            "items": [
                {
                    ...
                    "motion_evidence": item.candidate.has_motion_evidence,
                    "evidence_score": round(item.candidate.evidence_score, 4),
                }
                for item in self.items
            ],
            ...
            "motion_evidence_ratio": round(self.motion_evidence_ratio, 4),
        }
...
class StrictWeightMatcher:
    ...
    Attributes:
        ...
        max_items: 조합당 최대 상품 수 (기본값 2)
        max_count_per_item: 상품당 최대 개수 (기본값 10)
        max_combinations: 최대 조합 수 (성능 제한, 기본값 100)
    """

    MULTI_KIND_MIN_CONFIDENCE = 0.18

    def __init__(
        self,
        tolerance: Optional[float] = None,
        max_items: Optional[int] = None,
        max_kinds: Optional[int] = None,
        ...
        self.max_kinds = (
            max_kinds if max_kinds is not None else app_config.weight.max_combination_kinds
        )
        ...
        self.multi_kind_min_confidence = float(
            app_config.weight.multi_kind_min_confidence
        )
        self.last_diagnostics: Dict[str, Any] = {}
        self.last_return_diagnostics: Dict[str, Any] = {}
    ...
    def find_valid_combinations(
        ...
        self.last_diagnostics = {
            ...
            "tolerance": round(float(self.tolerance), 1),
            "input_candidates": len(candidates),
            "active_products": 0,
            "max_items": int(self.max_items),
            "max_kinds": int(self.max_kinds),
            "multi_kind_min_confidence": round(
                float(self.multi_kind_min_confidence), 4
            ),
            "extraction_stats": {},
            "candidate_products": [],
            "nearest_residuals": [],
            ...
            "rejected_combination_counts": {},
            "reason": None,
        }

        logger.info("[STRICT] ========== 엄격 무게 매칭 ==========")
        logger.info(
        ...
        if target_weight < self.tolerance:
            self.last_diagnostics["reason"] = "target_below_tolerance"
        ...
        self.last_diagnostics["active_products"] = len(active_product_map)
        ...
        self.last_diagnostics["extraction_stats"] = dict(extraction_stats)
        self.last_diagnostics["candidate_products"] = [
            {
                "class_id": product.class_id,
                "name": product.name,
                "weight": round(float(product.weight), 1),
                "stock": int(product.stock),
                "vision_confidence": round(float(product.vision_confidence), 4),
                "unit_price": int(product.unit_price),
                "vote_count": int(product.vote_count),
                "raw_vote_count": int(product.raw_vote_count),
                "source": product.source,
                "motion_evidence": product.has_motion_evidence,
                "top_motion_passed": bool(product.top_motion_passed),
                "side_motion_passed": bool(product.side_motion_passed),
                "motion_gate_passed": bool(product.motion_gate_passed),
                "weight_gate_passed": product.weight_gate_passed,
                "evidence_score": round(float(product.evidence_score), 4),
            }
            for product in candidate_products
        ]
        self.last_diagnostics["nearest_residuals"] = [
            self._candidate_residual(product, target_weight)
            for product in candidate_products
        ]
        logger.info(
        ...
        if not candidate_products:
            self.last_diagnostics["reason"] = "no_candidate_products"
        ...
        if not valid_combinations:
            self.last_diagnostics["reason"] = "no_valid_combinations"
            logger.info(
                f"[STRICT] 유효 조합 없음 (target={target_weight:.1f}g ± {self.tolerance}g)"
            )
            return []

        valid_combinations = self._filter_risky_combinations(valid_combinations)
        if not valid_combinations:
            self.last_diagnostics["reason"] = "all_combinations_rejected"
            logger.warning(
                "[STRICT][reason=all_combinations_rejected] "
                f"rejected_counts={self.last_diagnostics['rejected_combination_counts']}"
            )
            return []

        has_static_single_match = any(
            combo.total_count == 1 and combo.motion_evidence_ratio <= 0.0
            for combo in valid_combinations
        )

        def combination_sort_key(
            ...
            motion_supported_multi = (
                ...
                and combo.motion_evidence_ratio > 0.0
            )
            return (
                0 if motion_supported_multi else 1,
                combo.total_count,
                combo.weight_error,
                -combo.evidence_score,
                combo.kind_count,
                -combo.match_score,
            )

        valid_combinations.sort(key=combination_sort_key)
        self.last_diagnostics["reason"] = "strict_match"
        self.last_diagnostics["valid_combinations"] = [
            combo.to_dict()
        ...
        for i, combo in enumerate(valid_combinations[:3]):
            ...
            logger.info(
                ...
                f"score={combo.match_score:.3f}, adjusted={combo.adjusted_score:.3f}"
            )
    ...
    def _candidate_residual(
        ...
        if product.weight <= 0:
            ...
            nearest_count = max(1, round(target_weight / product.weight))
            nearest_count = min(product.stock, nearest_count)
            residual = abs(target_weight - product.weight * nearest_count)
        return {
            "class_id": product.class_id,
            "name": product.name,
    ...
    def _extract_candidate_products(
        ...
        for candidate in candidates:
            ...
            cp = CandidateProduct(
                ...
                weight_gate_passed=getattr(candidate, "weight_gate_passed", None),
            )
            result.append(cp)

        return result, stats

    def _filter_risky_combinations(
        self,
        combinations: List[ValidCombination],
    ) -> List[ValidCombination]:
        filtered: List[ValidCombination] = []
        rejected: List[dict[str, Any]] = []
        counts: Dict[str, int] = {}

        for combo in combinations:
            reason = self._risky_combination_reason(combo)
            if reason is None:
                        ...
                        "items": [
                            {
                                ...
                                "vision_confidence": round(
                                    float(item.candidate.vision_confidence), 4
                                ),
                                "vote_count": int(item.candidate.vote_count),
                                "raw_vote_count": int(item.candidate.raw_vote_count),
                            }
                            for item in combo.items
                        ],
                        "total_weight": round(float(combo.total_weight), 1),
                        "weight_error": round(float(combo.weight_error), 1),
                    }
                )

        self.last_diagnostics["rejected_combinations"] = rejected
        self.last_diagnostics["rejected_combination_counts"] = counts
        return filtered

    def _risky_combination_reason(
        self,
        combo: ValidCombination,
    ) -> Optional[str]:
        if combo.kind_count > self.max_kinds:
            return "too_many_kinds"

        if any(
            self._unsupported_small_repeat_fragment(item)
            for item in combo.items
        ):
            return "unsupported_small_repeat_fragment"

        if combo.kind_count >= 2:
            weak_items = [
                item
                for item in combo.items
                if item.candidate.vision_confidence < self.multi_kind_min_confidence
            ]
            if weak_items:
                return "low_confidence_combo"

        return None

    @staticmethod
    def _unsupported_small_repeat_fragment(item: CombinationItem) -> bool:
        product = item.candidate
        if float(product.weight) >= 200.0 or int(item.count) < 2:
            return False
        if StrictWeightMatcher._source_includes_regular_vision(product.source):
            return False
        if not product.has_motion_evidence:
            return True

        votes = max(int(product.vote_count), int(product.raw_vote_count))
        confidence = float(product.vision_confidence)
    ...
    def _backtrack(
        ...
        # 조합 크기 제한
        current_count = sum(item.count for item in current_combo)

        if len(current_combo) >= self.max_kinds or current_count >= self.max_items:
            return
        ...
        for i in range(start_idx, len(candidates)):
            ...
            max_count = min(
                cand.stock,
                self.max_count_per_item,
                self.max_items - current_count,
            )
    ...
    def find_return_combination(
        ...
        self.last_return_diagnostics = {
            "target_weight": round(float(delta_weight), 1),
            "tolerance": round(float(self.tolerance), 1),
            "per_item_tolerance": round(
                float(app_config.weight.same_product_count_tolerance_grams), 1
            ),
            "accepted": False,
            "reason": None,
        }
        if delta_weight <= self.tolerance:
            self.last_return_diagnostics["reason"] = "target_below_tolerance"
            return None
        ...
        if not candidates:
            self.last_return_diagnostics["reason"] = "no_return_candidates"
            return None

        result: Dict[int, int] = {}
        found = self._backtrack_return(candidates, delta_weight, 0, result)
        if not found:
            self.last_return_diagnostics["reason"] = "no_return_combination"
            return None

        expected_weight = sum(
            weight * result.get(product_id, 0)
            for product_id, weight, _ in candidates
        )
        used_count = sum(result.values())
        residual = abs(float(delta_weight) - expected_weight)
        allowed_residual = self._return_allowed_residual(used_count)
        self.last_return_diagnostics.update(
            {
                "accepted": True,
                "reason": "return_combination_match",
                "expected_weight": round(float(expected_weight), 1),
                "residual": round(float(residual), 1),
                "allowed_residual": round(float(allowed_residual), 1),
                "total_count": int(used_count),
                "combo": {
                    int(product_id): int(count)
                    for product_id, count in result.items()
                },
            }
        )
        return result

    def _return_allowed_residual(self, used_count: int) -> float:
        if used_count <= 1:
            return float(self.tolerance)
        return float(self.tolerance) + (
            float(app_config.weight.same_product_count_tolerance_grams)
            * int(used_count)
        )
    ...
    def _backtrack_return(
        ...
        # 기저 조건: 허용 오차 내 → 성공
        used_count = sum(result.values())
        if result and abs(remaining) <= self._return_allowed_residual(used_count):
            return True

        # Pruning: remaining이 음수 → 초과
        if remaining < -self._return_allowed_residual(max(1, used_count)):
            return False
        ...
        # 깊이 제한
        if used_count >= max(self.max_items, 5):
            return False
        ...
        for count in range(1, capped + 1):
            ...
            branch_count = used_count + count
            if used_weight > remaining + self._return_allowed_residual(branch_count):
