...
from model_service.engine.models import CountEstimate, EnsembleResult
...
class WeightBasedCountCalculator:
    ...
    def calculate(
        ...
        Args:
            candidates: Multi-View Ensemble 결과 (Top-K)
        ...
        for candidate in candidates:
            ...
            if active_product is not None:
                ...
                if self.use_stock_limit and stock <= 0:
                    logger.info(
                ...
                if count <= 0:
                    logger.info(
                        f"[COUNT][reason=estimated_count_below_one] {product_name} "
                        f"(delta={abs_weight:.1f}g, unit_weight={product_weight:.1f}g, stock={stock})"
                    )
                    continue
        ...
        for est in estimates[:candidate_limit]:
            logger.info(
    ...
    def calculate_combination(
        ...
        for candidate in candidates[:candidate_limit]:
            if candidate.combined_confidence < min_multi_kind_confidence:
                logger.debug(
                    f"[COMBINATION] reject class_id={candidate.class_id}: "
                    f"low_confidence_combo confidence={candidate.combined_confidence:.3f}"
                )
                continue
            active_product = active_product_map.get(candidate.class_id)
        ...
        if len(product_candidates) >= 2:
            for (cand1, prod1), (cand2, prod2) in combinations(product_candidates, 2):
                ...
                for count1, count2 in iterproduct(range(1, max_count1 + 1), range(1, max_count2 + 1)):
                    if count1 + count2 > max_combination_size:
                        continue
                    if (
                        self._unsupported_small_repeat_fragment(cand1, prod1, count1)
                        or self._unsupported_small_repeat_fragment(cand2, prod2, count2)
                    ):
                        logger.debug(
                            "[COMBINATION] reject unsupported small repeat fragment: "
                            f"{prod1.name}x{count1} + {prod2.name}x{count2}"
                        )
                        continue
                    ...
                    if error <= tolerance:
                        ...
                        count_penalty = 1.0 - ((total_count - 2) * 0.1)  # 2개일 때 최고
                        ...
                        current_key = (total_count, error, -avg_confidence, -score)
    ...
    @staticmethod
    def _unsupported_small_repeat_fragment(
        candidate: EnsembleResult,
        product: Any,
        count: int,
    ) -> bool:
        if float(getattr(product, "weight", 0.0) or 0.0) >= 200.0 or int(count) < 2:
            return False
        if WeightBasedCountCalculator._source_includes_regular_vision(
            getattr(candidate, "source", "vision")
        ):
            return False

        motion_gate_passed = bool(
            getattr(candidate, "motion_gate_passed", False)
            or getattr(candidate, "top_motion_passed", False)
            or getattr(candidate, "side_motion_passed", False)
        )
