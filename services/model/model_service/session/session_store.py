...
class SessionStore:
    ...
    def save(self, session_id: str, data: SessionData) -> None:
        ...
        with self._lock:
            ...
            if previous is not None:
                if self._is_expected_lifecycle_update(previous, data):
    ...
    @staticmethod
    def _is_expected_lifecycle_update(
        ...
        allowed_transitions = {
            "processing": {"processing", "waiting", "complete", "error"},
            "waiting": {"waiting", "complete", "error"},
        }
        return current.status in allowed_transitions.get(previous.status, set())
