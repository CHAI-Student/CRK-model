...
class FrameExtractionDiagnostics:
    ...
    final_branch: str = ""
    decoder: str = "auto"
    ffprobe_attempts: int = 0
    ffprobe_wait_ms: float = 0.0
...
class StreamingFrameExtractor:
    ...
    def __init__(
        ...
        self._ffprobe_attempts = 0
        self._ffprobe_wait_ms = 0.0
    ...
    def _probe_video(self) -> bool:
        ...
        ready_max_wait = float(config.video.ready_max_wait_seconds)
        poll_interval = max(0.001, float(config.video.ready_poll_interval_seconds))
        started_at = time.monotonic()
        deadline = started_at + ready_max_wait
        attempt = 0
        self._ffprobe_attempts = 0
        self._ffprobe_wait_ms = 0.0
        ...
        while True:
            attempt += 1
            self._ffprobe_attempts = attempt
            ...
            if result.returncode != 0:
                ...
                if "Invalid data" in stderr_msg:
                    now = time.monotonic()
                    remaining = deadline - now
                    if remaining > 0:
                        sleep_seconds = min(poll_interval, remaining)
                        logger.warning(
                            "[FFPROBE] File may still be writing, "
                            f"retrying in {sleep_seconds:.3f}s... "
                            f"(attempt={attempt}, elapsed={now - started_at:.3f}s, "
                            f"max_wait={ready_max_wait:.3f}s)"
                        )
                        if sleep_seconds > 0:
                            time.sleep(sleep_seconds)
                        self._ffprobe_wait_ms = (
                            time.monotonic() - started_at
                        ) * 1000
                        continue

                    self._ffprobe_wait_ms = (now - started_at) * 1000
                    logger.warning(
                        "[FFPROBE] File still invalid after readiness polling: "
                        f"attempts={attempt}, wait_ms={self._ffprobe_wait_ms:.1f}, "
            ...
            self._ffprobe_wait_ms = (time.monotonic() - started_at) * 1000
            logger.info(
                f"[FFPROBE] ready: attempts={self._ffprobe_attempts}, "
                f"wait_ms={self._ffprobe_wait_ms:.1f}, "
                f"frames={self._total_frames}, duration={self._duration:.3f}s"
            )
            return True
    ...
    def _new_diagnostics(
        ...
        return FrameExtractionDiagnostics(
            ...
            decoder=decoder,
            ffprobe_attempts=self._ffprobe_attempts,
            ffprobe_wait_ms=round(self._ffprobe_wait_ms, 1),
        )
    ...
    @staticmethod
    def _log_diagnostics(prefix: str, diagnostics: FrameExtractionDiagnostics) -> None:
        message = (
            ...
            f"ffprobe_attempts={diagnostics.ffprobe_attempts} "
            f"ffprobe_wait_ms={diagnostics.ffprobe_wait_ms:.1f}"
        )
