...
- The default engine path is now `models/0204_morning.engine`.
- Current local verification gate on 2026-06-04:
  - `uv run --no-sync ruff check services/model scripts`
  - `uv run --no-sync pytest services/model/tests -q` (`351 passed`)
  These local checks cover static, unit, and contract regressions; they do not
  prove Jetson runtime readiness.
...
cd CRK-model
source .venv/bin/activate
python scripts/live_engine_preview.py --model models/0204_morning.engine --source 0 --display-backend ffplay
```
...
## Direct Jetson Runtime

This repository now supports the direct Jetson Python service path only. Runtime
settings are loaded from `.env` by `model_service.core.config.Settings`.
...
To replace an engine file, copy the new TensorRT `.engine` into `models/` and
set `MODEL__VISION__YOLO_MODEL_PATH` in `.env` if the filename changes.
